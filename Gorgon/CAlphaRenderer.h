	...
	namespace wustl_mm {
		namespace Visualization {	
			...
			*/
			...
			struct fittedHelix {
				vector<int> fittedHelixBondIndices;
				vector<Vector3DFloat> fittedHelixBondPositions;
				float coilHeight;
				float coils;
				float orthoDist;
				float helixError;
	...
	//			void GetSSEHunterAtoms(Volume * vol, NonManifoldMesh_Annotated * skeleton, float resolution, float threshold, float correlationCoeff, float skeletonCoeff, float geometryCoeff);
				...
				vector< vector<Vector3DFloat> > originalHelices;
				vector<float> strandCycles;
				vector<float> orthoDistances;
				...
				//void scoreHelix(vector<Vector3DFloat> currentStrand, );
				...
				vector<Vector3DFloat> ordLines;
				vector<float> helixScoreThresholds;
				vector<int> quadPts;
				vector<float> quadNormals;
				vector<Quad> quads;
				vector<float> vertexPositions;
				vector<Vector3DFloat> maxCoord;
				vector<Vector3DFloat> minCoord;
				vector<Vector3DFloat> saddleCoord;

				vector<float> helixTurnHeights;

				vector<float> maxPointSaliencies;
				vector< fittedHelix > fittedHelices;

				vector<float> maxPointIntensities;
				vector<float> maxPointEigenvalues;
				vector<float> maxPointEigenvalues0;
				vector<float> maxPointEigenvalues1;

				vector<float> minPointSaliencies;

				vector<float> minPointIntensities;
				vector<float> minPointEigenvalues;
				vector<float> minPointEigenvalues0;
				vector<float> minPointEigenvalues1;
				vector< vector<Vector3DFloat> > normalizedHelices;

				vector<float> saddlePointSaliencies;

				vector<float> saddlePointIntensities;
				vector<float> saddlePointEigenvalues;
				vector<float> saddlePointEigenvalues0;
				vector<float> saddlePointEigenvalues1;

				vector< vector<unsigned long long> > helices;
				vector<unsigned long long> bondAtm1s;
				vector<unsigned long long> bondAtm2s;

				vector<int> maxCurveIndices1;
				vector<int> maxCurveIndices2;

				vector<float> maxCurveSaliencies;
				vector<float> maxCurveIntensities;
				vector<float> maxCurveEigenvalues;
				vector<Vector3DFloat> axisPts;
				vector< vector<float> > maxCurveSepPts;

				vector<float> coefsA;
				vector<float> coefsB;
				vector<float> coefsC;
				//vector<float> coefsD;
				vector<float> curveMinsX;
				vector<float> curveMinsY;
				vector<float> curveMinsZ;
				vector<float> curveMaxsX;
				vector<float> curveMaxsY;
				vector<float> curveMaxsZ;

				vector<float>curveMeanXs;
			 	vector<float>curveMeanYs;
			 	vector<float>curveMeanZs;
				//vector<Vector3DFloat> pseudoatoms;
				//void toggleHideOn(bool display);
				vector<float> eigenValues0;
				vector<float> eigenValues1;
				vector<float> eigenValues2;

				vector<Vector3DFloat> helixMaxs;
				vector<Vector3DFloat> helixMins;
				vector< vector<Vector3DFloat> > foundHelices;
				vector<int> foundHelixIndices;

				vector<float> normalizedMinXs;
				vector<float>	normalizedMinYs;
				vector<float>	normalizedMinZs;
				vector<float>	normalizedMaxXs;
				vector<float>	normalizedMaxYs;
				vector<float>	normalizedMaxZs;

				vector<Matrix4> rotationMatrices;
				vector<float> rotAngles;
	...
	void CAlphaRenderer::findCurveMinsMaxes() {
		for(int i = 0; i < maxCurveSepPts.size(); i++) {
			vector<float> currentSeg = maxCurveSepPts[i];
			float xMin = 10000.0;
			float yMin = 10000.0;
			float zMin = 10000.0;
			float xMax = -10000.0;
	...
	void CAlphaRenderer::inRangeOfSkeleton() {
		for(int i = 0; i < maxCurveSepPts.size(); i++) {
			vector<float> currentStrand = maxCurveSepPts[i];
	...
	void CAlphaRenderer::makeMin(int index, bool minB, float saliency1, float saliency2, float saliency3, float intensity, float eigenvalue, float eigenValue0, float eigenValue1) {
		...
		Vector3DFloat findMean(vector<Vector3DFloat> points) {
			float sumX = 0.0;
			float sumY = 0.0;
			float sumZ = 0.0;
		...
		// top
  		...
  		static bool sortZValue(Vector3DFloat lhs, Vector3DFloat rhs)
  		{
			...
			void CAlphaRenderer::sortMaxCurveDimensions() {
				//cout << "called " << endl;
				vector< tuple<float, float, float, int, int> > eigenvalues;
				//vector<float> eigenV0;
				//vector<float> eigenV1;
				//vector<float> eigenV2;
				for(int i = 0; i < bonds.size(); i++) {
					//if(bonds[i].maxOn) {
						tuple<float, float, float, int, int> eigenValueTuple(bonds[i].eigenValue0, bonds[i].eigenValue1, bonds[i].eigenvalue, i, 0);
				...
				for(int i = 0; i < quads.size(); i++) {
					tuple<float, float, float, int, int> eigenValueTuple(quads[i].eigenValue0, quads[i].eigenValue1, quads[i].eigenValue, i, 1);
					eigenvalues.push_back(eigenValueTuple);
				}

				for(int i = 0; i < saddlePointEigenvalues.size()/3; i++) {
					tuple<float, float, float, int, int> eigenValueTuple(saddlePointEigenvalues[3*i], saddlePointEigenvalues[3*i+1], saddlePointEigenvalues[3*i+2], i, 2);
					eigenvalues.push_back(eigenValueTuple);
				}

				for(int i = 0; i < maxPointEigenvalues.size()/3; i++) {
					tuple<float, float, float, int, int> eigenValueTuple(maxPointEigenvalues[3*i], maxPointEigenvalues[3*i+1], maxPointEigenvalues[3*i+2], i, 3);
					eigenvalues.push_back(eigenValueTuple);
				}

				for(int i = 0; i < minPointEigenvalues.size()/3; i++) {
					tuple<float, float, float, int, int> eigenValueTuple(minPointEigenvalues[3*i], minPointEigenvalues[3*i+1], minPointEigenvalues[3*i+2], i, 4);
					eigenvalues.push_back(eigenValueTuple);
				}
			...
			int CAlphaRenderer::findHelices() {
				...
				vector< tuple<unsigned long long, unsigned long long> > maxCurveSegments;
				//for(int i = 0; i < bonds.size(); i++) {
					//if(bonds[i].maxOn) {
						//tuple<unsigned long long, unsigned long long> segmentTuple(bonds[i].GetAtom0Ix(), bonds[i].GetAtom1Ix());
				...
				for(int i = 0; i < bondAtm1s.size(); i++) {
					...
					tuple<unsigned long long, unsigned long long> segmentTuple(bondAtm1s[i], bondAtm2s[i]);
					maxCurveSegments.push_back(segmentTuple);
				} 
				int i = 0;
				while(maxCurveSegments.size() > 0) {
					vector<unsigned long long> currentHelix;
					int j = 0;
					tuple<unsigned long long, unsigned long long> currentSegment = maxCurveSegments[j];
					...
					while(segment2 != -1) {
						...
						for(int k = 0; k < maxCurveSegments.size(); k++) {
							tuple<unsigned long long, unsigned long long> nextSegment = maxCurveSegments[k];
					...
					if(currentHelix.size() > 4) {
						...
						/**
						vector<Vector3DFloat> firstOrderDifferences;
						...
						**/
						/**
						vector<Vector3DFloat> secondOrderDifferences;
						...
						**/
						/**
						vector<Vector3DFloat> atomPositions;
						for(int i = 0; i < currentHelix.size(); i++) {
						...
						vector<Vector3DFloat> meanDifferences;
				...
				for(int i = 0; i < helices.size(); i++) {
						vector<unsigned long long> currentH = helices[i];
...
//Divide into buckets
			...
			void CAlphaRenderer::findMaxCurves() {
				...
				**/
				...
				for(int j = 0; j < helices.size(); j++) {
					...
					vector<unsigned long long> currentMaxCurve = helices[j];
			...
			/**
			void CAlphaRenderer::fitHelix(vector<unsigned long long> currentStrand) {
			...
			void CAlphaRenderer::fitHelices() {
				for(int i = 0; i < helices.size(); i++) {
					vector<unsigned long long> currentHelix = helices[i];
			...
			**/
			void CAlphaRenderer::interpolateHelixPoints() {
				...
				for(int i = 0; i < helices.size(); i++) {
								//	vector<float> currentSeg = maxCurveSepPts[i];
									...
									vector<Vector3DFloat> helixNormalizedPos;
								...
								//vector< Vector3DFloat > currentStrand = normalizedHelices[i];
			...
			void CAlphaRenderer::scoreHelix(int strandIndex) {
				...
				vector<Vector3DFloat> thisStrand =  normalizedHelices[strandIndex];
				float avgA = 0.0;
				float avgB = 0.0;
				float avgW = 0.0;
				float avgSigma = 0.0;
				//vector<Vector3DFloat> thisStrand = currentStrand;
				...
				//vector<Vector3DFloat> originalPositions;
				...
				vector<Vector3DFloat> validHelixPts;
					...
					//OpenGLUtils::SetColor(0.6, 0.9, 0.1, 1.0);
							...
							if(displayHelix) {
								for(int i = 0; i < foundHelices.size(); i++) {
									...
									vector<float> currentStrand = maxCurveSepPts[helixIndex];
									if(orthoDistMin < orthoDistances[i] && orthoDistMax > orthoDistances[i]) {
										...
										if(helixScoreThresholds[i] < helixThreshold && helixTurnHeights[i] < helixCoilHeight) {
											vector<Vector3DFloat> foundHelix = foundHelices[i];
							...
							**/
							...
							if(displayHelixPoints) {
								...
								for(int i = 0; i < originalHelices.size(); i++) {
									vector<Vector3DFloat> thisStrand = originalHelices[i];
									if(helixScoreThresholds[i] < helixThreshold && helixTurnHeights[i] > helixCoilHeight) {
										for(int j = 0; j < thisStrand.size(); j++) {
									...
									if(displayCurrentHelix) {
										if(displayHelix) {
											...
											if(orthoDistMin < orthoDistances[i] && orthoDistMax > orthoDistances[i]) {
												...
												//if(helixScoreThresholds[i] < helixThreshold && helixTurnHeights[i] < helixCoilHeight) {
														vector<Vector3DFloat> helPositions = fittedHelices[i].fittedHelixBondPositions;
...
/**
									...
									int helixIndex = bonds[i].helixIndex;
										...
										if(orthoDistMin < orthoDistances[bonds[i].helixIndex] && orthoDistMax > orthoDistances[bonds[i].helixIndex]) {
											...
											if(helixScoreThresholds[bonds[i].helixIndex] < helixThreshold && helixTurnHeights[bonds[i].helixIndex] < helixCoilHeight) {
												vector<Vector3DFloat> foundHelix = foundHelices[bonds[i].helixIndex];
												if((currentStrand.size()/3)-1  >= helixSegmentThreshold) {
													for(int j = 0; j <  foundHelix.size(); j++) {
														DrawSphere(foundHelix[j], 0.03);
												}
							...
							if(displayHelixPoints) {
								...
								//for(int i = 0; i < foundHelixIndices.size(); i++) {
								if(bonds[i].helixIndex != -1) {
									vector<unsigned long long> bonds[bondIndex].helixIndices;
